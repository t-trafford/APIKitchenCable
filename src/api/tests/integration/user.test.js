...
describe('Users API', async () => {
  ...
  beforeEach(async () => {
    ...
    if (!count) {
      await User.remove({});
      await User.insertMany([dbUsers.branStark, dbUsers.jonSnow, dbUsers.admin]);
    }
    ...
    customUserAccessToken = (await User.findAndGenerateToken(dbUsers.admin)).accessToken;
  });
    ...
    it('should get all users', () => {
      return request(app)
        ...
        .then(async (res) => {
          ...
          expect(includesadmin).to.be.true;
        });
    });
